...
- For a complex request, choose the smallest implementation that satisfies the requested behavior. State material simplifications and their limits; do not substitute a partial solution for an explicit requirement. Resolve routine choices from current evidence.
...
Lazy code without its check is unfinished. Non-trivial logic — a branch, a loop, a parser, a money or security path — needs the smallest meaningful runnable check, reusing existing coverage when sufficient and using the target project's existing test placement, runner, and commands. Do not introduce a new framework, fixture layer, or test convention. Trivial one-liners need no test; YAGNI applies to tests too.
