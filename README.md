...
Kiokuko does not own Codex's plugin or tool inventory. New runs bind only required `kiokukoSkills`, containing exact available names from Kiokuko's managed six-Skill manifest; Kiokuko's MCP tool set comes from the server's internal manifest. Optional `clientInventory` is capped, recommendation-only, never stored or run-bound, and may change between calls without environment recovery. Codex hosts must never use `ALL_TOOLS.map(...)`; optional MCP inventory selects only `ALL_TOOLS.filter(tool => tool.name.startsWith("mcp__"))` and never relabels built-in tools.

### Recovering when plan-start environment information is missing or changed

The environment information used here is the bound set of Kiokuko-managed Skills, not the AI client's complete Skill and tool inventory. The host collects `kiokukoSkills` automatically; the user never constructs JSON. If this owned set is missing from the plan or changed since task preparation, Kiokuko records only a pause that suppresses automatic continuation, then stops before effects. Changes to optional `clientInventory` do not cause this recovery.
...
### Recovering when Oduno and Zenki select different Skill requirements

New Oduno ideals persist their complete Skill requirements. After the environment binding is confirmed, Kiokuko compares that set with Zenki's route-expanded requirements by canonical Skill name, purpose set, and requiredness. Ordering and duplicates do not matter. A difference pauses before Zenki discovery, advisory consumption, plan receipts, plan persistence, contract revision, or implementation.

The user sees only added, omitted, and changed Skill names, purposes, and requiredness, followed by four choices with advantages and disadvantages:

- **Use Enno-Oduno's original Skill set** — Zenki replans in the same attempt and must match the ideal requirements exactly.
- **Use the Skill set identified by Zenki** — only the exact compared plan may be resubmitted and normal confirmation still applies.
- **Revalidate both Skill sets** — the same attempt returns to Oduno ideal derivation with intake preserved and the previous ideal and draft replaced.
- **Cancel** — the attempt ends without a replacement.

Zenki is recommended only when it preserves every required Oduno Skill and purpose and all required Zenki Skills are executable; otherwise revalidation is recommended. Legacy stored ideals without Skill requirements remain readable and skip this comparison.
