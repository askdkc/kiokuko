...
function taggedEntries(database: SqliteDatabase, workspace: string, tags: string[], limit = 12): EntryRecord[] {
  ...
  while (found.length < limit) {
    ...
    `).all<{ id: string; updated_at: string }>(workspace, ...tags,
      ...(cursor ? [cursor.updated_at, cursor.updated_at, cursor.id] : []), Math.max(32, limit));
    ...
    for (const row of rows) {
      ...
      if (isRetrievableEntry(database, entry) && entry.status !== 'superseded'
        && entry.tags.some(tag => requestedTags.has(tag))) found.push(entry);
