...
function attemptRow(database: SqliteDatabase, identity: AgentTaskSkillDiscoveryAttemptIdentity): StoredAttemptRow | undefined {
  ...
  `).get<StoredAttemptRow>(identity.runId, identity.requestDigest);
}
...
function activeAttemptRow(database: SqliteDatabase, runId: string): StoredAttemptRow | undefined {
  return database.prepare(`
    SELECT run_id, request_digest,
           reserved_query_count, reserved_selection_count,
           consumed_query_count, consumed_selection_count,
...
function validateAttemptRow(row: StoredAttemptRow, identity?: AgentTaskSkillDiscoveryAttemptIdentity): void {
  if (typeof row.run_id !== 'string'
    ...
    || !boundedBudget(row.reserved_query_count, MAX_TOTAL_SKILL_QUERIES)
    || !boundedBudget(row.reserved_selection_count, MAX_EXTERNAL_SKILLS)
...
function usedDiscoveryBudget(database: SqliteDatabase, runId: string): {
  ...
  if (queryBudget > MAX_TOTAL_SKILL_QUERIES || selectionBudget > MAX_EXTERNAL_SKILLS) integrity();
...
export function claimAgentTaskSkillDiscoveryAttempt(
  ...
  if (!boundedText(identity.runId, 256)
    ...
    || !boundedBudget(requestedBudget.queryBudget, MAX_TOTAL_SKILL_QUERIES)
    || !boundedBudget(requestedBudget.selectionBudget, MAX_EXTERNAL_SKILLS)) {
    throw new KiokukoError('VALIDATION_ERROR', 'Task Skill discovery attempt identity is invalid');
  }
  return withImmediateTransaction(database, () => {
    const existing = attemptRow(database, identity);
    if (existing === undefined) {
      const active = activeAttemptRow(database, identity.runId);
      if (active !== undefined) {
        validateAttemptRow(active);
        throw new KiokukoError('CONFLICT', 'Task Skill discovery is already in progress or did not complete');
      }
      const used = usedDiscoveryBudget(database, identity.runId);
      const queryBudget = Math.min(requestedBudget.queryBudget, MAX_TOTAL_SKILL_QUERIES - used.queryBudget);
      const selectionBudget = Math.min(requestedBudget.selectionBudget, MAX_EXTERNAL_SKILLS - used.selectionBudget);
      ...
      `).run(identity.runId, identity.requestDigest, grantedQueryBudget, grantedSelectionBudget, new Date().toISOString());
...
export function completeAgentTaskSkillDiscoveryAttempt(
  ...
  return withImmediateTransaction(database, () => {
    ...
    `).get<{ state: unknown; summary_json: unknown }>(serialized, validated.queries.length, validated.selected.length, finishedAt, identity.runId, identity.requestDigest);
