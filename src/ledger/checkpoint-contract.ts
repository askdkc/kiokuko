...
export const CHECKPOINT_TOOL_DESCRIPTION =
  `Store one final batch of durable facts, decisions, lessons, preferences, or references as untrusted candidate memory. Only items supplied in memories become new memory entries; evidence and feedback alone create no memory. The response reports storedMemoryCount. ${CHECKPOINT_CONTRACT_FRAGMENT} After a successful terminal checkpoint, call no more tools and return the final response. Defaults to the current project. Use Curator for verified global promotion. Clearly general interaction knowledge may be captured separately as untrusted global candidates with a portability reason. Do not resubmit memories already saved by memory_capture. Secret-like content is rejected.`;
