...
test('generated Web UI exposes locale detection, persistence, selection, and translated accessibility hooks', () => {
  ...
  assert.match(WEB_HTML, /<div id="curator-selection-count" class="curator-selection-count curator-selection-summary">/);
  ...
  assert.match(WEB_HTML, /actionRoot\.append\(selectAll, clearSelection, apply\)/);
  assert.match(WEB_HTML, /curator-actions \.button/);
  assert.match(WEB_HTML, /curator-dialog/);
  assert.match(WEB_HTML, /curatorApplySelected/);
  assert.match(WEB_HTML, /curatorSelected/);
