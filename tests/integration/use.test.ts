...
test('use creates binding and AGENT.md, then is unchanged on repeat', async () => {
  ...
  const agentBefore = await readFile(path.join(root, 'AGENT.md'), 'utf8');
  ...
  assert.equal(await readFile(path.join(root, 'AGENT.md'), 'utf8'), agentBefore);
  await access(databasePath);
});

test('use creates version-2 binding, result, and repository metadata', async () => {
  ...
  assert.equal(binding.templateVersion, 2);

  const database = openConnection(databasePath);
  try {
    assert.equal(database.prepare('SELECT agent_template_version AS version FROM repositories WHERE repository_id = ?').get<{ version: number }>(result.repositoryId)?.version, 2);
  } finally {
    database.close();
  }
});
...
test('use upgrades a version-1 binding and managed block without changing identity or human bytes', async () => {
  ...
  await writeFile(path.join(root, 'AGENT.md'), `human before\r\n${oldManagedBlock}\r\nhuman after\r\n`);

  const upgraded = await useRepository({ root, databasePath });
  const upgradedAgent = await readFile(path.join(root, 'AGENT.md'), 'utf8');
  ...
  assert.equal(upgraded.templateVersion, 2);
  assert.equal(upgradedBinding.repositoryId, first.repositoryId);
  assert.equal(upgradedBinding.workspace, 'upgrade-workspace');
  assert.equal(upgradedBinding.templateVersion, 2);
  assert.match(upgradedAgent, /^human before\r\n/);
  assert.match(upgradedAgent, /<!-- kiokuko-template-version: 2 -->/);
  assert.match(upgradedAgent, /\r\nhuman after\r\n$/);

  const repeatedAgent = await readFile(path.join(root, 'AGENT.md'), 'utf8');
  ...
  assert.equal(await readFile(path.join(root, 'AGENT.md'), 'utf8'), repeatedAgent);
...
test('use dry-run does not create database or repository files', async () => {
  ...
  await assert.rejects(access(path.join(root, 'AGENT.md')));
  await assert.rejects(access(databasePath));
});

test('no-agent-file still upgrades version metadata without creating AGENT.md', async () => {
...
test('use preserves human content and rejects malformed markers', async () => {
  ...
  await import('node:fs/promises').then(({ writeFile }) => writeFile(path.join(root, 'AGENT.md'), 'human\n'));
  const data = await mkdtemp(path.join(tmpdir(), 'kiokuko-data-'));
  ...
  await writeFile(path.join(root, 'AGENT.md'), malformed);
  await assert.rejects(useRepository({ root, databasePath: path.join(data, 'db.sqlite3') }), /malformed/i);
  assert.equal(await readFile(path.join(root, 'AGENT.md'), 'utf8'), malformed);
