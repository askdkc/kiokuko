...
test('use creates version-10 binding, result, and repository metadata', async () => {
  ...
  assert.equal(result.templateVersion, 10);
  assert.equal(binding.templateVersion, 10);

  const database = openConnection(databasePath);
  try {
    assert.equal(database.prepare('SELECT agent_template_version AS version FROM repositories WHERE repository_id = ?').get<{ version: number }>(result.repositoryId)?.version, 10);
  } finally {
    database.close();
  }
});
...
test('use rejects a future binding version before mutating files or repository metadata', async () => {
  ...
  const agentPath = path.join(root, 'AGENTS.md');
  const binding = JSON.parse(await readFile(bindingPath, 'utf8')) as Record<string, unknown>;
  const futureBinding = `${JSON.stringify({ ...binding, templateVersion: 11 }, null, 2)}\n`;
  await writeFile(bindingPath, futureBinding);
  const agentBefore = await readFile(agentPath, 'utf8');

  await assert.rejects(
    useRepository({ root, databasePath }),
    /binding templateVersion 11 is newer than supported version 10/i,
  );
...
test('use rejects future managed-block versions at current and retired agent paths without partial mutation', async () => {
  for (const target of ['current', 'retired'] as const) {
    ...
    await assert.rejects(
      ...
      /agentFile uses managed template version 11, newer than supported version 10/i,
    );
...
test('use rejects future database version metadata and restores force-rebind file writes', async () => {
  ...
  const database = openConnection(databasePath);
  try {
    database.prepare('UPDATE repositories SET agent_template_version = 11 WHERE repository_id = ?')
  ...
  try {
    ...
    assert.equal(repositories[0]?.agentTemplateVersion, 11);
    assert.equal(
