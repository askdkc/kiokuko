...
import { NudgeDeliveryService } from '../../src/gateway/nudge-delivery-service.js';
...
function checkpoint(service: CheckpointMutationService, runId: string, idempotencyKey: string, request: Record<string, unknown>) {
...
test('delivers verify-after-mutation once and preserves the original nudge on exact replay', async () => {
  ...
  try {
    ...
    const service = new CheckpointMutationService(database, () => now);
    const request = { changedPaths: ['src/app.ts'] };
    const first = checkpoint(service, opened.runId, 'verify-checkpoint', request);
    assert.equal(first.preliminaryRecommendations.some((item) => item.code === 'VERIFY_AFTER_MUTATION'), true);
    const firstNudge = new NudgeDeliveryService(database, () => now).deliver({
      runId: opened.runId,
      idempotencyKey: 'verify-checkpoint',
      throughSequence: first.acceptedThrough,
      projection: first.projection,
      recommendations: first.preliminaryRecommendations,
    });
    ...
    const replayNudge = new NudgeDeliveryService(database, () => now).deliver({
      runId: opened.runId,
      idempotencyKey: 'verify-checkpoint',
      throughSequence: replay.acceptedThrough,
      projection: replay.projection,
      recommendations: replay.preliminaryRecommendations,
    });
    ...
    assert.equal(unrelated.preliminaryRecommendations.some((item) => item.code === 'VERIFY_AFTER_MUTATION'), true);
    assert.equal(new NudgeDeliveryService(database, () => now).deliver({
      runId: opened.runId,
      idempotencyKey: 'verify-unrelated',
      throughSequence: unrelated.acceptedThrough,
      projection: unrelated.projection,
      recommendations: unrelated.preliminaryRecommendations,
    }), null);
  } finally {
    database.close();
  }
...
test('rejects checkpoint replay when a stored nudge code no longer matches its occurrence', async () => {
  ...
  try {
    ...
    assert.throws(
      () => new NudgeDeliveryService(database, () => now).deliver({
        runId: opened.runId,
        idempotencyKey: 'binding-checkpoint',
        throughSequence: replay.acceptedThrough,
        projection: replay.projection,
...
test('creates a new unresolved-failure occurrence after the previous episode is resolved', async () => {
  ...
  try {
    ...
    const secondNudge = new NudgeDeliveryService(database, () => now).deliver({
      runId: opened.runId,
      idempotencyKey: 'failure-b-checkpoint',
      throughSequence: second.acceptedThrough,
      projection: second.projection,
      recommendations: second.preliminaryRecommendations,
    });
