...
Migration 010 adds `nudge_deliveries`, the content-free presentation history for deterministic checkpoint nudges. Migration 011 adds database guards for the supported nudge policy, code/priority pairs, and bounded JSON snapshots. Migration 012 upgrades persisted scoped context deliveries from released `context-ranking-v2` and `context-ranking-v3` identities to `context-ranking-v4` during setup. It preserves delivery items and updates context feedback, ledger memory links, and purge-audit references to the new delivery identity; an invalid legacy delivery aborts the migration transaction rather than being discarded or repaired implicitly.
